...
//   
  ...
  useEffect(() => {
    console.log(CurrentEmail,"CE", Newemail,"NE", Name,"N", CurrentPassword,"CP",NewPassword,"NP",Number,"N");
    console.log(userdetails,"lll");
  }, [CurrentEmail, Newemail, Name, CurrentPassword,NewPassword,userdetails,Number]);
            ...
            if (checkMail(Newemail)) {
                if (NewPassword.length>=5) {
                   console.log("login successfully");
                    axios.put("http://localhost:1111/updateUsers/", data)
            .then((response) => {
    ...
    console.log(data);
      ...
      if (checkMail(CurrentEmail)) {
        if (CurrentPassword.length >= 5) {
          await axios.post("http://localhost:1111/userEmail/", data)
            .then((response) => {
              console.log(response.data);
              ChangeUsrdetails(response.data);
              call();
